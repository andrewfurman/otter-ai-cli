...
- Otter's search backend can return varying partial results to identical requests. The CLI mitigates this by issuing several `advanced_search` calls per query (`--tries N`, default 5), spaced a few seconds apart, and unioning their results (deduplicated by OTID). Keyword search results are therefore best-effort; date-bounded speaker/date searches are the reliable ones.
- Speaker-only with no dates uses windowed `advanced_search` over recent 90-day windows, walking back in time and unioning each window's results, with an overall time budget (`--max-seconds`, default 120). Progress and coverage are printed to stderr, and partial results are returned if the budget is reached early.
